...
async function loadRemote(){
  const keys=['products','texts','contacts','design','users','orders','site_docs','a_login','a_pwd','admin_login','admin_pwd'];
  keys.forEach(k=>{try{const raw=localStorage.getItem(k);if(raw!==null)CACHE[k]=JSON.parse(raw);}catch{}});
  try{
...
function updateAuthButtons(){
  const u=currentUser();
  const login=document.querySelector('[data-auth-login]'); const reg=document.querySelector('[data-auth-register]');
  if(!login||!reg)return;
  if(u){
...
async function init(){
  await loadRemote();
  applyDesign();applyTexts();setActiveNav();updateCartCount();updateAuthButtons();renderPage();mountAdminPanel();
...
function openUserAuth(mode='login'){
  ...
  const html=`<div class="modal-backdrop" onclick="backdropClose(event)"><div class="modal">
    ...
    <p class="muted-note" style="margin-top:18px">Важно: в этой версии регистрация хранится через фронтенд и Firestore/localStorage. Это не замена полноценной серверной авторизации.</p>
  </div></div>`;
  setModal(html);
}
...
function renderAdminContent(tab){
  ...
  if(tab==='design')root.innerHTML=adminHeader('Дизайн','Шрифты и размеры. Сохраняется только по кнопке «Сохранить дизайн».')+designAdminHtml();
...
function settingsAdminHtml(){return `<div class="admin-form-block"><h3>Смена логина и пароля</h3><div class="admin-row"><div class="form-field"><label>Новый логин</label><input class="form-input" id="set-login" value="${esc(adminLogin())}"></div><div class="form-field"><label>Новый пароль</label><input class="form-input" id="set-pwd" type="password" placeholder="Оставьте пустым, если не менять"></div></div><div class="admin-save-line"><button class="primary-btn" onclick="saveAdminSettings()">Сохранить настройки</button><span class="save-msg" id="settings-save-msg">Сохранено</span></div></div><div class="admin-form-block"><h3>Сброс локальной корзины</h3><p class="muted-note">Удаляет только корзину в текущем браузере.</p><button class="danger-btn" onclick="clearCart()">Очистить корзину</button></div>`;}
...
window.openCart=openCart;window.addProductToCart=addProductToCart;window.changeCartQty=changeCartQty;window.removeCartItem=removeCartItem;window.clearCart=clearCart;window.openCheckout=openCheckout;window.submitCheckout=submitCheckout;
...
window.saveAdminTexts=saveAdminTexts;window.saveAdminContacts=saveAdminContacts;window.saveAdminSettings=saveAdminSettings;
